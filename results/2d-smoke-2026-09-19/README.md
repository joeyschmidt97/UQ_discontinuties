...
all 2 test cases are shown together. **Every integer point count from 4 to 150 is scored on one nested trajectory per method and seed.**
...
6. [All error diagnostics](figures/06-holistic-error-types.png) â€” one pooled subplot per error type, with every method overlaid.

Or open [the single scrolling report](index.html), which includes all 6 sheets.

## How to read the figures

- **Placement/reference/maps:** one representative paired seed (0), at the largest point count (150). N printed on placement panels is the actual number of paid samples, including four shared corners. Every method spends exactly this many points.
...
Methods qualifying on every seed of every case: **Progressive grid, Triangles, GP uncertainty, GP gradient, GP grad/unc 50/50, GP Matern .5 uncertainty, VWRS, VURS**.
