...
def main():
    ...
    parser.add_argument("--cases", nargs="+", choices=CASES, default=["on-plane", "offset-peaks", "jump"])
    ...
    if args.plots_only:
        ...
        return
    if args.quick:
        args.cases, args.seeds, args.budgets = ["on-plane"], [0], [32, 64]
                ...
                for budget in sorted(set(args.budgets)):
                    for name in args.arms:
                        if (case, seed, budget, name) in completed:
                            continue
                        row = dict(case=case, seed=seed, budget=budget, arm=name)
                        start = time.perf_counter()
                        obs = Observations(surface, budget)
                        try:
                            predict, metadata = run_arm(name, obs, seed)
                            row.update(score(surface, obs, test), status="ok", metadata=metadata)
                            if predict is not None and args.native_test_size:
                                # Native prediction is secondary; never blocks design scoring.
                                try:
                            ...
                            row.update(status="unavailable", reason=str(exc), n=len(obs.x))
                        except Exception as exc:
                            row.update(status="failed", reason=f"{type(exc).__name__}: {exc}", n=len(obs.x))
                        row["seconds"] = time.perf_counter()-start
                        payload["rows"].append(row)
                        save(result_path, payload)
                        print(f"{case:12} seed={seed} cap={budget:4} {name:10} {row['status']:11} n={row['n']:4} "
